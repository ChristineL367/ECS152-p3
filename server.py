    ...
    def handshake(self, connection): #basically accept
        try:
            ...
            if message2.get_type() == "ACK" and connection.got_syn == 1:
                ...
                return self.curconnection.data_port
            else:
    ...
    def data(self, data_port):
        ...
        try:
            while (1):
                ...
                message = self.bits_to_header(packet)
                if message.get_type() == "FIN":
                    self.data_closeconnection(0, address[0], address[1], data_port.getsockname()[
                ...
                response = TCP_header(address[1], 0, 0, 0, 0, 0, "Pong", data_port.getsockname()[1])
                packet = response.get_bits()

                log.append([address[0], address[1], "DATA", len(packet)])
                self.send_packet(packet, address[0], address[1],data_port)


        except KeyboardInterrupt:
            print("Server Keyboard Interruption")
            self.data_closeconnection(1, address[0], address[1], data_port.getsockname()[1])
...
class dataThread(Thread):
    def __init__(self, server_init, own_sock):
        Thread.__init__(self)
        self.server_init = server_init
        self.own_sock = own_sock
    def run(self):

        self.server_init.data(self.own_sock)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        own_sock = server_init.handshake(new_connection)

        if new_connection.connected == 1:

            print("connected to new client")
            new_data = dataThread(server_init, own_sock)
            new_data.daemon = True
            new_data.start()
            if server_init.curconnection.closed == 1:
                break
