...
def accept(welcome_socket, port): #basically accept
    try:
        ...
        if message.get_type() == "SYN":
            ...
            got_syn = 1
            # log.append([port, address[1], "SYNACK", len(synack.get_bits()), time.time()])
    ...
    try:
        ...
        if mask & selectors.EVENT_READ:
            ...
            if packet and address:
                ...
                # log.append([sock.getsockname()[1], address[1], "DATA", len(packet)], time.time())
...
def welcoming_closeconnection(welcome_socket, putah, cur_seq, cur_ack, address, dst_port, src_port):
    if putah == 1:
        ...
        while ack != True:
            ...
            # log.append([src_port, dst_port, "FIN", len(message.get_bits()), time.time()])
        ...
        # log.append([src_port, dst_port, "ACK", len(message.get_bits()), time.time()])
        welcome_socket.sendto(message.get_bits(), (address, dst_port))
