        ...
        try:
            while (1):
                ...
                if message_syn.get_type() == "SYN":
                    ...
                    synack = TCP_header(connection.data_port.getsockname()[1], 0, message_syn.sequence_num + 1, 0, 0, 0,
                                        "", self.server_port)
            ...
            self.welcoming_closeconnection(1, cur_seq, cur_ack, address[0], address[1],
                                           self.welcome_socket.getsockname()[1])
            return 0
        ...
        with open(os.path.join(str(port), output_file), 'w') as f:
            ...
            try:
                while (1):
                    ...
                    log.append([self.curconnection.data_port.getsockname()[1], address[1], "DATA", len(response.get_bits())])
                    jit = random.uniform(0, 1)
...
if __name__ == '__main__':
    ...
    handshaken = server_init.handshake(new_connection, packet_loss, jitter)
    if handshaken == 1:
        print("connection established") 
    if new_connection.connected == 1:
        print("connected to new client")
        server_init.data(packet_loss, jitter, output_file)
